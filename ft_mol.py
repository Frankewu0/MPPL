...
from molnet_pipeline import MolNetDataset
...
train_dataset = MolNetDataset(task_name, mode="train")
train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=8, drop_last=True, collate_fn=collator)
# train_loader = DataLoader(train_dataset, batch_size=4, shuffle=True, num_workers=8, drop_last=True, collate_fn=collator)
if task_mode == "classification":
    wp, wn = train_dataset.wp, train_dataset.wn
max_v, min_v = train_dataset.max_v, train_dataset.min_v

test_dataset = MolNetDataset(task_name, mode="test")
...
model_params = [
    ...
    # {'params': model.head.parameters(), 'lr': 2e-4, "weight_decay": 0.},
    # {'params': model.head.parameters(), 'lr': 1e-3, "weight_decay": 5e-2},
    
    # {'params': model.mpim.parameters(), 'lr': 2e-4, "weight_decay": 1e-2},
...
if optims == 'adan':
    ...
    optimizer = Adan(model_params, betas=(0.98, 0.92, 0.99), eps=1e-8, max_grad_norm=5.)
    # optimizer = Adan(model_params, betas=(0.97, 0.92, 0.95), eps=1e-8, max_grad_norm=5.)
    # optimizer = Adan(model_params, betas=(0.98, 0.92, 0.99), max_grad_norm=1.)
elif optims == 'sgd':
    optimizer = optim.SGD(model_params, momentum=0.9, weight_decay=wd)
elif optims == 'adamw':
    optimizer = optim.AdamW(model_params, betas=(0.9, 0.999))
elif optims == 'adam':
    ...
    for step_id, batched_data in enumerate(tqdm(train_loader)):
        ...
        if task_mode == "classification":
            ...
            for i, lab in enumerate(label):
                if lab == 0:
                    loss_main[i] = loss_main[i] * (wp/(wp+wn)) # * 0.1  # best
                else:
        ...
        # loss = loss_main*100. + loss_attr
        # loss = loss_main*10. + loss_attr
        # loss = loss_main*100. + loss_attr * 0.1
        loss = loss_main # single
